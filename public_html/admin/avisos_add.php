...
class cavisos_add extends cavisos {
	...
	function LoadDefaultValues() {
		...
		$this->url_aviso->Upload->DbValue = NULL;
		$this->url_aviso->OldValue = $this->url_aviso->Upload->DbValue;
		$this->url_aviso->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->url_aviso);
	...
	function AddRow($rsold = NULL) {
		...
		if (!$this->url_aviso->Upload->KeepFile) {
			$this->url_aviso->Upload->DbValue = ""; // No need to delete old file
